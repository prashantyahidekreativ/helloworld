...
class HrHolidaySummaryReport(osv.AbstractModel):
    ...
    def _get_header_info(self, start_date, holiday_type):
        return {
            'start_date': datetime.strptime(start_date, DEFAULT_SERVER_DATE_FORMAT).strftime('%d-%m-%Y'),
            'end_date': (datetime.strptime(start_date, DEFAULT_SERVER_DATE_FORMAT) + relativedelta(days=59)).strftime('%d-%m-%Y'),
