...
class account_extension(models.Model): 
    ...
    def compute_all(self, price_unit, currency=None, quantity=1.0, product=None, partner=None):
        ...
        for tax in self:
            ...
            if (tax.sub_tax_ids  and tax.price_include and not self._context.get('tax')) or (self._context.get('pos_tax') and tax.sub_tax_ids):
                    ret = tax.sub_tax_ids._compute_amount(price_unit, currency, quantity, product, partner)
                    #total_excluded = ret['total_excluded']
                    ret = round(ret, prec)
                    base = price_unit-ret
                    if self._context.get('pos_tax') and not tax.price_include:
                        base = price_unit+ret
                    #total_included = ret['total_included']
                    #tax_amount = total_included - total_excluded
                    #taxes += ret['taxes']
...
class kts_account_fiscal_position(models.Model):
    ...
    form_type= fields.Selection([('form_c','Form C'), ('form_h','Form H'), ('form_i','Form I')], 'Form Type',select=True)
...
class kts_account_invoice_line_tax_include(models.Model):
    ...
    def _compute_price(self):
        ...
        self.price_subtotal = price_subtotal_signed = taxes['total_excluded'] if taxes else 1.0 * price
...
class kts_purchase_order(models.Model):
    ...
    @api.depends('order_line.price_subtotal','tax_line_ids','freight_charges','packing_charges','fiscal_position_id','order_line.discount',)
    def _compute_order_tax(self):
        taxes_grouped = self.get_taxes_values()
        tax_lines = self.tax_line_ids.browse([])
        for tax in taxes_grouped.values():
    ...
    date_planned = fields.Datetime(string='Scheduled Date', inverse='', required=True, select=True, oldname='minimum_planned_date') 
