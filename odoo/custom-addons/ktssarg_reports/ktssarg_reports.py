...
from openerp.addons.report_aeroo.currency_to_text import currency_to_text
import operator
...
class kts_reports(models.Model):
    ...
    name= fields.Char('Name', size=256, select=True)
       # 'file_path':  fields.many2one('document.directory', 'File Folder', domain=[('storage_type','=','filestore')],help='Printed files will be saved to this folder'),
...
def do_print_setup(self, cr, uid, ids, vals,attachment=False,partner_id=False,context={}):
    this=self.browse(cr,uid,ids[0],context)
    uiModelAndReportModelSame=context.get('uiModelAndReportModelSame', True)
    if not uiModelAndReportModelSame:
        this=context.get('this',False)
        if this:
            del context['this']               
    if not this:
        raise osv.except_osv('Info', 'Print context is not set')  
    ...
    if attachment:
        ...
        if time_zone:
           ...
           time_now=now_time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        else:
            time_now=fields.Datetime.now()
        attachment_str="((object.name)+'_' + ('%s') )"%(time_now)
    
    report_id = report_xml.search(cr, uid,
                         [('model','=', vals['model']),('report_name','=', report_name),('report_type','=', 'aeroo')])
    for i in report_xml.browse(cr,uid,report_id):
        report_xml.write(cr,uid,[i.id],{'report_name':i.name},context=context)
        
    report_template = report_xml.search(cr, uid,
                         [('model','=', vals['model']),('name','=', vals['name']),('report_type','=', 'aeroo')])
    
    if not report_template:
        raise osv.except_osv('Info', 'Document print \"%s\" of model \"%s\" is not configured!'%(report_name,vals['model']))                    
    
    template=report_xml.browse(cr,uid,report_template[0])   
    out_format=template.out_format.id
    
    if context and context.get('out_format', False):
        out_format=context.get('out_format', False)    
    
    if report_template:
        report_xml.write(cr,uid,report_template,{'tml_source':'file','report_name':report_name,'attachment':attachment_str,'out_format':out_format},context=context)
    else:      
    ...
    data = {'model':vals['model'], 'ids':ids, 'id':ids[0], 'report_type': 'aeroo','attachment':'TESTNEW'}
    context.update({'kts_reprots': True,'report_name': report_name,'partner_id':partner_id})    
    return {
...
def do_print_setup_new(self,vals,report_name,tmpl_name):
    report_xml = self.env['ir.actions.report.xml']
    report_id = report_xml.search([('model','=',vals['model']),('report_name','=',tmpl_name),('report_type','=', 'aeroo')])
...
def create_one2many_id(self,cr,uid,obj,vals,context):
    one2manylist=[]
    param_obj = self.pool.get(obj)
    obj_id=param_obj.create(cr, uid, vals, context=context)
    one2manylist.append((4,obj_id,False))
    return create_one2many_id  
...
def initializ_all_to_false(self,cr, uid, ids,context=None):
    val={}
    for key in self._columns:
        if key not in self._defaults:
            val.update({key:False})
    return val
    

def get_report_types(self, cr, user,context=None):
    report_types = []
...
def get_currency_code(self,cr,uid,currency=False,id=None):
    if currency is None or False and id==None:
        return ''
    if id==None:
        cur_id=self.pool.get('res.currency').search(cr,uid,[('name','=',currency)])       
        if cur_id:
            id = cur_id[0]
    cur = self.pool.get('res.currency').browse(cr,uid,id)
    symbol = cur.symbol
    return symbol
    

def getSelectedItem(self,cr,uid,field,value,context):
    if value=='' or value==False:
        return ''
    return dict(self.fields_get(cr,uid,[field],context)[field]['selection'])[value]

def modification_date(filename):
    t = os.path.getmtime(filename)
    return datetime.fromtimestamp(t)

class kts_report_uitlity(osv.osv):
    _name = "kts.report_uitlity"
    _description = "Utility Functions for Report"
    _inherit = "kts.report"
    
    def get_report_types(self, cr, user,context=None):
        return get_report_types(self, cr, user,context);
...
class kts_report_aeroo(models.Model):
    ...
    def name_get(self, cr, uid, ids, context=None):
         ...
         for d in self.browse(cr, uid, ids, context=context):
            if d.report_type !='aeroo':
                continue
            res.append((d.id, d.name ))
         return res
     
    def to_update_file_name(self, cr, uid, ids, context={}):
        this =self.browse(cr,uid,ids[0],context)
        report_ids = [] if this.update_query else self.get_report_keys(cr, uid, ids,context)
        query_ids = get_report_types(self, cr, uid,context) if this.update_query else []
        
        message='no report found for update/create. \n\n'
        create_report=[]
        for d in report_ids:
            report_id=self.pool.get('ir.actions.report.xml').search(cr,uid,
            ...
            if report_id:
                self.pool.get('ir.actions.report.xml').write(cr,uid,report_id,d,context)
            else:
                self.pool.get('ir.actions.report.xml').create(cr,uid,d,context)
        r='' 
...
#         report_class = self.pool.get('kts.financial_reports')
#         update_query=[]
#         create_query=[]
#         updated_query=[]
#         i=0
...
#         for d in query_ids:
#             file_name= curr_dir+'/reports/'+d[0]+'.txt' 
#             try:  
...
#             
#             report_query=report_class.search(cr,uid,[('report_type','=',d[0]),('show_query','=',True)],context=context)
#             if report_query:
#                 query=report_class.browse(cr,uid,report_query[0],context)
#                 c = datetime.strptime(query.create_date, '%Y-%m-%d %H:%M:%S')    
#                 w = datetime.strptime(query.write_date, '%Y-%m-%d %H:%M:%S') if query.write_date else datetime.strptime('2001-07-20 19:55:51', '%Y-%m-%d %H:%M:%S')   
#                 if (c >last_modified) or (w and w > last_modified):
#                     continue
#                 update_query.append(report_query[0])
#                 updated_query.append(d[0])
#             else:
#                 create_query.append(d[0])
#             i=i+1
#         
#         created_queries=[report_class.create(cr,uid,{'report_type':d,'show_query':True},context) for d in create_query ]
#         total_update_query=update_query+created_queries
#         x=[report_class.update_report_query(cr, uid, [d], context={}) for d in total_update_query]
#         if total_update_query:
#             ret=''
#             r=''
#             for i in updated_query:
#                 ret = ret+'\n'+getSelectedItem(report_class,cr,uid,'report_type',i,context) 
#             updated_query=ret
#             for i in create_query:
#                 r = r+'\n'+getSelectedItem(report_class,cr,uid,'report_type',i,context) 
#             create_query=r
#             
#             message = message+'\n\nFollowing queries created:\n    %s \n\n and following queries updated:\n    %s' % (create_query,updated_query)
#         else:
#             message= message+'\n\nno query found for update/create.'
#             
        super(kts_report_aeroo, self).write(cr, uid, ids, {'update_info':message}, context)
        return True
...
class kts_report_sale_order(models.Model):
    ...
    def to_print_quotation_sale_order(self, cr, uid, ids, context={}):
        this = self.browse(cr, uid, ids[0], context=context)
             #context.update({'context_dir':SALE_ORDER})
        if this.tax_type != 'gst':
        ...
        return do_print_setup(self,cr, uid, ids, {'name':name, 'model':'sale.order','report_name':report_name},
                False,this.partner_id.id, context)
      
    
    def to_print_sale_order(self, cr, uid, ids, context={}):
        this = self.browse(cr, uid, ids[0], context=context)
             #context.update({'context_dir':SALE_ORDER})
...
class kts_account_payment_report(models.Model):
    ...
    def to_print_reciept_voucher(self, cr, uid, ids, context={}):
        this = self.browse(cr, uid, ids, context=context) 
        if this.partner_type=='customer':
           report_name='receipt'
           report_name1='Receipt'
        elif this.partner_type=='supplier':
             report_name='voucher'
             report_name1='Voucher'
            
        context.update({'this':this, 'uiModelAndReportModelSame':False})
        return do_print_setup(self,cr, uid, ids, {'name':report_name1, 'model':'account.payment','report_name':report_name},
                False,False,context)
...
class kts_sale_reports(models.Model):
    ...
    def to_print_sale(self, cr, uid, ids, context={}):
           this = self.browse(cr, uid, ids, context=context) 
           ret=False
           if this.report_type=='total_so_summary':
...
class kts_account_invoice_report(models.Model):
    ...
    def to_print_invoice(self, cr, uid, ids, context={}):
        this = self.browse(cr, uid, ids[0], context=context)
             #context.update({'context_dir':SALE_ORDER})
        
        if this.type == 'out_invoice' and this.tax_type=='gst':
            report_name= 'customer_invoice_gst'
            name='Customer Invoice GST'
        elif this.type == 'in_invoice' and this.tax_type=='gst':
            report_name= 'supplier_invoice_gst'
            name='Supplier Invoice GST'     
        elif this.type=='out_refund'  and this.tax_type=='gst':
            report_name= 'credit_note_gst'
            name='Credit Note GST'
        elif this.type=='in_refund' and this.tax_type=='gst':
            report_name= 'debit_note_gst'
            name='Debit Note GST' 
        elif this.type=='out_invoice':
           report_name= 'customer_invoice'
           name='Customer Invoice'          
        elif this.type=='out_refund':
            report_name= 'credit_note'
            name='Credit Note'
        elif this.type=='in_invoice':
            report_name= 'supplier_invoice'
            name='Supplier Invoice'
        elif this.type=='in_refund':
            report_name= 'debit_note'
            name='Debit Note'
                 
        context.update({'this':this, 'uiModelAndReportModelSame':False})
        return do_print_setup(self,cr, uid, ids, {'name':name, 'model':'account.invoice','report_name':report_name},
                False,this.partner_id.id, context)
...
class kts_mrp_production_report(models.Model):
    
    _inherit ='mrp.production'
        
    jobcard_no=fields.Char('JobCard No')
    jobcard_printed = fields.Boolean('Job Card Printed',default=False)  
    partial_job_card_print=fields.Boolean('Partial Job Card Printed', default=False)
    
    def action_produce(self, cr, uid, production_id, production_qty, production_mode,lot_id1, wiz=False, context=None):
        res=super(kts_mrp_production_report, self).action_produce(cr, uid,production_id, production_qty, production_mode, lot_id1, wiz, context )
        record = self.browse(cr, uid, production_id)
        if record.jobcard_printed:
            record.write({'jobcard_printed':False})
        for line in record.move_lines:
            product=line.product_id
            raw_id=line.raw_material_production_id
            loc_dest_id=line.location_dest_id.id
            cr.execute('select sum(product_uom_qty),sum(partial_job_card_qty) from stock_move where raw_material_production_id=\'%s\' and product_id=\'%s\' and state=\'done\' and location_dest_id=\'%s\'  group by product_id, raw_material_production_id', (raw_id.id,product.id,loc_dest_id)) 
            total=cr.fetchall()
            total_qty=float(total[0][0] if total[0][0] else 0.0 )
            al_iss_qty=float(total[0][1] if total[0][1] else 0.0)
            if al_iss_qty - total_qty >= 0:
                reserve1=al_iss_qty - total_qty
                line.write({'partial_job_card_qty':reserve1})
        for line1 in record.move_lines2:
            if line1.partial_job_card_qty > line1.product_qty:
                 line1.write({'partial_job_card_qty':line1.product_qty})          
        return res
    
    def to_print_jobcard(self, cr, uid, ids, context={}):
           this = self.browse(cr, uid, ids, context=context) 
           report_name= 'job_card'
           report_name1='Job Card'
           jobcard_qty=this.product_qty
           this.write({'jobcard_qty':jobcard_qty})
           this.write({'jobcard_printed':True})     
           this.write({'jobcard_no':self.pool.get('ir.sequence').next_by_code(cr,uid,'kts.jobcard.no',context)})
           context.update({'this':this, 'uiModelAndReportModelSame':False})
           return do_print_setup(self,cr, uid, ids, {'name':report_name1, 'model':'mrp.production','report_name':report_name},
                  True,False,context)
    
    def to_print_partial_jobcard(self, cr, uid, ids, context={}):
           this = self.browse(cr, uid, ids, context=context) 
           report_name= 'partial_job_card'
           report_name1='Partial Job Card'
           this.write({'jobcard_no':self.pool.get('ir.sequence').next_by_code(cr,uid,'kts.jobcard.no',context)})
           this.write({'partial_job_card_print':True})     
           context.update({'this':this, 'uiModelAndReportModelSame':False})
           return do_print_setup(self,cr, uid, ids, {'name':report_name1, 'model':'mrp.production','report_name':report_name},
                  True,False,context)
    ...
    def get_val_recs(self):
        ...
        i=0
        for line in self.move_lines:
            if not self.reserve_flag:
                raise UserError(_('Please Reserve First'))
            already_issued_qty=0.0
            i+=1
            product=line.product_id
            raw_id=line.raw_material_production_id
            loc_dest_id=line.location_dest_id.id
            self.env.cr.execute('select sum(product_uom_qty),sum(partial_job_card_qty) from stock_move where raw_material_production_id=\'%s\' and product_id=\'%s\' and location_dest_id=\'%s\' group by product_id, raw_material_production_id', (raw_id.id,product.id,loc_dest_id)) 
            total=self.env.cr.fetchall()
            total_qty=float(total[0][0] if total[0][0] else 0.0 )
            al_iss_qty=float(total[0][1] if total[0][1] else 0.0)
            if not line.partial_job_card_qty:
               line.string_availability_info
               string=str(line.string_availability_info)
               reserve=float(re.search(r'\d+\.\d+',string).group())
               reserve1=reserve
               line.write({'partial_job_card_qty':reserve1})
            else:
                already_issued_qty=line.partial_job_card_qty
                string=str(line.string_availability_info)
                reserve=float(re.search(r'\d+\.\d+',string).group())
                reserve1=reserve - already_issued_qty
                line.write({'partial_job_card_qty':reserve})
            ...
            lines.append({
                         ...
                         'req_qty':total_qty,
                         'iss_qty':reserve1,
                         'bal_qty':total_qty -(reserve1+al_iss_qty ),
                         'al_iss_qty':al_iss_qty if al_iss_qty > 0.0 else 0.0
                          })
        self.write({'reserve_flag':False})
        return lines
    ...
    def get_move_lines(self):
        ...
        for line in self.move_lines:  
            ...
            self.env.cr.execute('select sum(product_uom_qty),sum(partial_job_card_qty) from stock_move  where raw_material_production_id=\'%s\' and product_id=\'%s\' and location_dest_id=\'%s\'  group by product_id, raw_material_production_id', (raw_id.id,product.id,loc_dest_id)) 
            total=self.env.cr.fetchall()
            self.env.cr.execute('select sum(partial_job_card_qty) from stock_move  where raw_material_production_id=\'%s\' and product_id=\'%s\'  group by product_id, raw_material_production_id', (raw_id.id,product.id)) 
            total_qty=float(total[0][0] if total[0][0] else 0.0 )
            total1=self.env.cr.fetchall()
            al_iss_qty=float(total1[0][0] if total1[0][0] else 0.0)
            self.env.cr.execute('select sum(partial_job_card_qty) from stock_move  where raw_material_production_id=\'%s\' and product_id=\'%s\' and location_dest_id != \'%s\'  group by product_id, raw_material_production_id', (raw_id.id,product.id,loc_dest_id)) 
            scrap_total=self.env.cr.fetchall()
            scrap_qty=float(scrap_total[0][0] if scrap_total else 0.0)
            
            if not line.partial_job_card_qty:
               line.string_availability_info
               string=str(line.string_availability_info)
               test_reserve=string.replace('.','',1).isdigit()
               if test_reserve:
                   test=float(string) 
                   if test>0.0:
                      raise UserError(_('Please Reserve Qty first'))
               reserve=float(re.search(r'\d+\.\d+',string).group())
               reserve1=reserve
               line.write({'partial_job_card_qty':reserve1})
            else:
            ...
            lines.append({
                         ...
                         'req_qty':total_qty,
                         'iss_qty':reserve1,
                         'bal_qty':total_qty - (reserve1+((al_iss_qty-scrap_qty) if al_iss_qty  > 0.0 else already_issued_qty)),
                         'al_iss_qty':al_iss_qty if al_iss_qty > 0.0 else 0.0,
                         'scrap_qty':scrap_qty
                          })
...
class kts_account_move_report(models.Model):
    ...
    def to_print_journal_voucher(self, cr, uid, ids, context={}):
           this = self.browse(cr, uid, ids, context=context) 
           report_name= 'journal_voucher'
           report_name1='Journal Voucher'
           context.update({'this':this, 'uiModelAndReportModelSame':False})
           return do_print_setup(self,cr, uid, ids, {'name':report_name1, 'model':'account.move','report_name':report_name},
                False,False,context)
...
class kts_report_purchase_order(models.Model):
    ...
    def to_print_purchase_order(self, cr, uid, ids, context={}):
        this = self.browse(cr, uid, ids[0], context=context)
             #context.update({'context_dir':SALE_ORDER})
        
        if this.state == 'purchase'and this.tax_type!='gst':
           report_name= 'purchase_order'
           name='Purchase Order'          
        elif this.state in ['draft','sent'] and this.tax_type!='gst':
        ...
        context.update({'this':this, 'uiModelAndReportModelSame':False})
        return do_print_setup(self,cr, uid, ids, {'name':name, 'model':'purchase.order','report_name':report_name},
                False,this.partner_id.id, context)
