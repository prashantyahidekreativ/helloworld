...
class kts_gst_res_company(models.Model):
    ...
    def onchange_state(self, cr, uid, ids, state_id, context=None):
        if state_id:
            state_obj=self.pool.get('res.country.state').browse(cr, uid, state_id, context)
            return {'value':{'country_id': state_obj.country_id.id,'state_code':state_obj.code }}
        return {}
        ...
        if result and self.tax_type == 'gst' and product:
            for line in result:
                ...
                if (line.gst_account_code_id.id != product.hsn_id.gst_account_id.id): 
                   ...
                   gst_acc_code=product.hsn_id.gst_account_id 
...
class kts_gst_account_invoice(models.Model):
    ...
    partner_shipping_id = fields.Many2one('res.partner', string='Shipping Address', readonly=True, required=True, states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, help="Invoice address for current sales order.")
